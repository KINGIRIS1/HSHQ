...
const PersonalProfile: React.FC<PersonalProfileProps> = ({
  ...
  const myRecords = useMemo(() => {
    const mainRecords = records.filter((r) => {
      ...
      if (isCheckerUser) {
        return (
          r.assignedTo === user.employeeId || r.checkedBy === user.employeeId
        );
      }
    ...
    const mappedArchives = archiveRecords
      .filter((r) => {
        ...
        if (isCheckerUser) {
          return (
            r.data?.assigned_to === user.employeeId ||
            r.data?.checked_by === user.employeeId
          );
        }
  ...
  const completedWorkRecords = useMemo(() => {
    let list = myRecords.filter(
      (r) => r.status === RecordStatus.COMPLETED_WORK,
    );
    return filterAndSort(list, searchTerm, sortConfig);
  }, [myRecords, searchTerm, sortConfig]);
            ...
            <>
              ...
              <div className="flex-1 md:flex-none text-center px-4 py-2 bg-cyan-50 rounded-lg border border-cyan-100 min-w-[100px]">
                <div className="text-2xl font-bold text-cyan-700">
                  {completedWorkRecords.length}
                </div>
                <div className="text-xs text-cyan-600 uppercase font-semibold">
                  Đã thực hiện
                </div>
              </div>
            </>
                        ...
                        <div className="flex justify-center gap-2">
                          ...
                          {isMeasurementTeam &&
                            ...
                            r.recordType !== "Công văn" && (
                              <button
                                onClick={() => {
                                  setAnnexTargetRecord(r);
                                  setIsAnnexModalOpen(true);
                                }}
                                className="px-2 py-1.5 bg-rose-50 text-rose-700 border border-rose-200 rounded-md hover:bg-rose-100 text-xs font-bold flex items-center gap-1 shadow-sm transition-all"
                                title="In phụ lục hợp đồng hệ thống"
                              >
                          ...
                          {activeTab === "pending" && (
                            <button
                              ...
                              title="Đánh dấu đã xong việc"
                              className="px-3 py-1.5 bg-cyan-600 text-white rounded-md hover:bg-cyan-700 text-xs font-bold flex items-center gap-2 shadow-sm transition-all"
                            >
                          ...
                          {activeTab === "pending_check" &&
                            ...
                            isChecker && (
                              <button
                                onClick={() => handleMarkAsChecked(r)}
                                title="Đánh dấu đã kiểm tra"
                                className="px-3 py-1.5 bg-teal-600 text-white rounded-md hover:bg-teal-700 text-xs font-bold flex items-center gap-2 shadow-sm transition-all"
                              >
